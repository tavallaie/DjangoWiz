...
from test.models import BuyableType
from rest_framework.test import APIClient
import os

class BuyableTypeAPITests(TestCase):
    fixtures = ['buyabletype_fixtures.json']

    def setUp(self):
        self.client = APIClient()
        self.model_list_url = reverse('buyabletype-list')
        self.model_detail_url = lambda pk: reverse('buyabletype-detail', args=[pk])
        self.instances = BuyableType.objects.all()
    ...
    def test_list_buyabletypes(self):
        response = self.client.get(self.model_list_url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), len(self.instances))

    def test_create_buyabletype(self):
        # Update data with the correct fields and values for BuyableType
        ...
        self.assertEqual(BuyableType.objects.count(), len(self.instances) + 1)

    def test_retrieve_buyabletype(self):
    ...
    def test_update_buyabletype(self):
    ...
    def test_partial_update_buyabletype(self):
        if not self.second_instance:
    ...
    def test_delete_buyabletype(self):
